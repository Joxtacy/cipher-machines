/**
 * Internal wiring of the PURPLE machine (Japanese Type B Cipher Machine,
 * Angooki Taipu B), reconstructed by Allied cryptanalysts and published in:
 *
 *   Wes Freeman, Geoff Sullivan and Frode Weierud, "PURPLE Revealed:
 *   Simulation and Computer-aided Cryptanalysis of Angooki Taipu B",
 *   Cryptologia 27(1), January 2003, pp. 1-43.
 *   https://cryptocellar.org/pubs/purple-revealed.pdf
 *
 * Each table holds one permutation per switch position: 25 positions, 6 levels
 * for the sixes switch and 20 levels for each of the three twenties stages.
 *
 * Kept 1-BASED exactly as printed in the paper, so the literals can be diffed
 * against the source by eye. `decryptTables()` converts to 0-based indices.
 * The published tables are the DECRYPT direction; the encrypt direction is the
 * inverse permutation of each row, computed in `encryptTables()` rather than
 * stored, so the two can never drift apart.
 *
 * Provenance caveat: these literals came via an OCR transcription of the paper,
 * so `tests/purple/data.test.ts` asserts every row is a well-formed permutation
 * rather than trusting them. See also docs/purple-plan.md.
 */

/** Sixes switch: 25 positions x 6 levels. */
export const SIXES: readonly (readonly number[])[] = [
	[2, 1, 3, 5, 4, 6],
	[6, 3, 5, 2, 1, 4],
	[1, 5, 4, 6, 2, 3],
	[4, 3, 2, 1, 6, 5],
...
/** Twenties stage I: 25 positions x 20 levels. */
export const TWENTIES_1: readonly (readonly number[])[] = [
...
/** Twenties stage II. */
export const TWENTIES_2: readonly (readonly number[])[] = [
...
/** Twenties stage III. */
export const TWENTIES_3: readonly (readonly number[])[] = [
...
/** The four switch tables, 1-based as published: sixes, then stages I-III. */
export const PUBLISHED_TABLES = [SIXES, TWENTIES_1, TWENTIES_2, TWENTIES_3] as const;

export const SWITCH_POSITIONS = 25;
export const SIXES_LEVELS = 6;
export const TWENTIES_LEVELS = 20;

/** Published tables shifted to 0-based indices — the decrypt direction. */
export function decryptTables(): number[][][] {
	return PUBLISHED_TABLES.map((table) => table.map((row) => row.map((v) => v - 1)));
}

/** Per-row inverse of the decrypt tables — the encrypt direction. */
export function encryptTables(): number[][][] {
	return decryptTables().map((table) =>
		table.map((row) => {
			const inverse = new Array<number>(row.length);
			for (let level = 0; level < row.length; level++) inverse[row[level]] = level;
			return inverse;
		})
	);
}
